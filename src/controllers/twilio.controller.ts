...
export const newMessage = catchedAsync(async (req: Request, res: Response) => {
  ...
  // const completion = await openAICompletion(chatBuscado, contexto, mensaje);
  // if (completion.choices[0].message.content !== null) {
  //   const chatRespuestaIA = await srvInsertarChat(
  ...
  // const countRespuestaIA = await srvChatToEmbeddings({
  //   cliente_id: chat.cliente_id,
  //   chat_id: chatRespuestaIA.chat_id,
  //   role: chatRespuestaIA.role,
  //   content: chatRespuestaIA.content,
  //   embedding: textoUsuarioEmbedding.data[0].embedding,
  // });

  // Registramos el chat

  // Registramos el chat y el embedding

  // console.log({ count });

  // const chatsUsuarioEmbeddings = await console.log({ chat });
